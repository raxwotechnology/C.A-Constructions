...
export default function MaterialRequestModal({ isOpen, onClose, defaultProjectId }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto text-slate-100">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <h3 className="text-lg font-bold text-white">Site Supervisor Material Request Form</h3>
        ...
        >
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl p-2.5 text-sm mt-1 focus:border-amber-500 focus:outline-none font-semibold text-amber-400"
            >
            ...
            {items.map((item, idx) => (
              <div key={idx} className="grid grid-cols-12 gap-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 items-center">
                <input
                  ...
                  className="col-span-6 bg-slate-900 border border-slate-800 text-white p-2 text-xs rounded-lg focus:outline-none"
                />
                <input
                  type="number" required min="1" placeholder="Qty"
                  value={item.requestedQty}
                  onChange={(e) => handleItemChange(idx, 'requestedQty', e.target.value)}
                  className="col-span-3 bg-slate-900 border border-slate-800 text-white p-2 text-xs rounded-lg focus:outline-none font-bold"
                />
                <input
