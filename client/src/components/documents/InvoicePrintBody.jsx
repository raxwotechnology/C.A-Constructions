  ...
  return (
    <div className="invoice-doc-inner" style={{ fontFamily: FONT, color: CLR.dark, fontSize: '10.5pt', lineHeight: 1.55 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px', marginBottom: '20px', pageBreakInside: 'avoid' }}>
        ...
        <div className="invoice-meta-block" style={{ padding: '14px 16px', background: CLR.bg, borderRadius: '8px', border: `1px solid ${CLR.border}`, wordBreak: 'break-word' }}>
          <style dangerouslySetInnerHTML={{__html: `@media (min-width: 480px) { .invoice-meta-block { text-align: right; } }`}} />
      ...
      {/* Payment History & Installment Ledger Table */}
      {Array.isArray(inv.payments) && inv.payments.length > 0 && (
        <div style={{ margin: '24px 0 20px', pageBreakInside: 'avoid' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: CLR.dark, borderRadius: '6px 6px 0 0' }}>
            <span style={{ fontSize: '8.5pt', fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              Payment History &amp; Installments
            </span>
            <span style={{ fontSize: '8.5pt', fontWeight: 600, color: '#38bdf8' }}>
              Total Paid: {formatMoney(inv.totalPaid || 0, currency)}
            </span>
          </div>
          <table className="doc-table" style={{ width: '100%', borderCollapse: 'collapse', border: `1px solid ${CLR.border}`, fontSize: '9pt' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                <th style={{ ...thStyle, textAlign: 'center', width: '36px' }}>#</th>
                <th style={{ ...thStyle, textAlign: 'left' }}>Date / Receipt</th>
                <th style={{ ...thStyle, textAlign: 'left' }}>Type</th>
                <th style={{ ...thStyle, textAlign: 'left' }}>Method &amp; Ref</th>
            ...
            <tbody>
              {(() => {
                ...
                return inv.payments.map((p, i) => {
                  ...
                  return (
                    <tr key={p._id || i}>
                      <td style={{ ...tdStyle, textAlign: 'center', color: CLR.light }}>{i + 1}</td>
                      <td style={{ ...tdStyle }}>
                        <span style={{ fontWeight: 600 }}>{p.date ? new Date(p.date).toLocaleDateString('en-LK') : '—'}</span>
                        {p.receiptNo && (
                          <span style={{ display: 'block', fontSize: '7.5pt', color: CLR.muted }}>{p.receiptNo}</span>
                        )}
                      </td>
                      <td style={{ ...tdStyle }}>
                        <span style={{
                          ...
                          fontSize: '7.5pt',
                          fontWeight: 700,
                          background: p.isAdvance ? '#f3e8ff' : '#e0f2fe',
                          color: p.isAdvance ? '#7e22ce' : '#0369a1',
                        }}>
                          {p.isAdvance ? 'Advance' : 'Installment'}
                        </span>
                      </td>
                      <td style={{ ...tdStyle, textTransform: 'capitalize' }}>
                        {(p.method || 'cash').replace('_', ' ')}
                        {p.reference && <span style={{ color: CLR.light, fontSize: '8pt' }}> ({p.reference})</span>}
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: '#15803d' }}>
                        {formatMoney(p.amount || 0, currency)}
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 600, color: '#b91c1c' }}>
