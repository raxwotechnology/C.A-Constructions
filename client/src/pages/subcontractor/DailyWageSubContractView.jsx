...
export default function DailyWageSubContractView() {
  ...
  // Clear all pending logs for a specific worker
  const clearWorkerPendingMutation = useMutation({
    mutationFn: async (logIds) => {
      const results = await Promise.all(
        logIds.map((id) => api.put(`/daily-wages/${id}`, { status: 'Paid' }))
      )
      return results
    },
    onSuccess: (_, variables) => {
      toast.success(`Cleared & marked ${variables.length} pending logs as Paid for worker!`)
    ...
    onError: () => {
      toast.error('Failed to clear worker pending payments.')
    },
  })
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mt-4 pt-3 border-t border-slate-200/80">
                      ...
                      {hasPending && (
                        <button
                          onClick={() => clearWorkerPendingMutation.mutate(worker.pendingLogIds)}
                          disabled={clearWorkerPendingMutation.isPending}
                        ...
                        >
                          <CheckCircle className="w-3.5 h-3.5" /> Clear All (Rs. {worker.pendingNetSubtotal.toLocaleString()})
                        </button>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-amber-50 border border-amber-300 rounded-xl p-3 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
              ...
              <div className="flex items-center gap-2">
                {workerPendingSummaryMap[selectedWorkerFilter]?.pendingLogsCount > 0 && (
                  <button
                    onClick={() =>
                      clearWorkerPendingMutation.mutate(workerPendingSummaryMap[selectedWorkerFilter].pendingLogIds)
                    }
                  ...
                  >
                    <CheckCircle className="w-3.5 h-3.5" /> Clear All Pending for {selectedWorkerFilter}
                  </button>
                )}
                <button
                  onClick={() => setSelectedWorkerFilter('')}
