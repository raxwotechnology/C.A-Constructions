...
export default function AdminEmployees() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: async () => {
      ...
      await qc.invalidateQueries({ queryKey: ['epf-summary'] })
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: async (_, variables) => {
      ...
      qc.setQueriesData({ queryKey: ['employees'] }, (old) => {
        if (!old) return old
        if (Array.isArray(old)) return old.filter((e) => String(e._id) !== String(deletedId))
        if (old.employees && Array.isArray(old.employees)) {
          return { ...old, employees: old.employees.filter((e) => String(e._id) !== String(deletedId)) }
        }
        return old
      })
