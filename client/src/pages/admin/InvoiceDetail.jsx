...
export default function InvoiceDetail({ invoiceId, onClose, onEdit }) {
  ...
  const recordPayMut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (updatedInv?.payments?.length) {
        const lastPay = updatedInv.payments[updatedInv.payments.length - 1]
        setSelectedReceiptPayment(lastPay)
      }
      reset()
    },
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
                <div className="p-4 border-b bg-slate-50 font-semibold text-slate-700 flex justify-between items-center">
                  <span>Payment &amp; Advance History</span>
                  <span className="text-xs text-slate-500 font-normal">{(inv?.payments || []).length} installment(s)</span>
                </div>
                <div className="overflow-x-auto hide-scrollbar">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    <thead className="border-b text-slate-500 bg-slate-50/50">
                      <tr>
                        <th className="p-3 font-semibold">Date / Receipt</th>
                        <th className="p-3 font-semibold">Type</th>
                        <th className="p-3 font-semibold">Method &amp; Ref</th>
                        <th className="p-3 font-semibold">Recorded By</th>
                        <th className="p-3 font-semibold text-right">Amount Paid ({inv?.currency})</th>
                        <th className="p-3 font-semibold text-right">Balance Due</th>
                        <th className="p-3 font-semibold text-center">Receipt</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {inv?.payments?.length === 0 ? (
                        ...
                        return inv?.payments?.map((p, i) => {
                          ...
                          const balAfter = typeof p.remainingBalanceAfter === 'number'
                            ? p.remainingBalanceAfter
                            : Math.max(0, Number(inv.total || 0) - running);
                          return (
                            <tr key={p._id || i} className="hover:bg-slate-50 transition-colors">
                              <td className="p-3 text-slate-600">
                                <span className="font-semibold text-slate-800">{new Date(p.date).toLocaleDateString('en-LK')}</span>
                                {p.receiptNo && (
                                  <span className="block text-[11px] font-mono text-secondary">{p.receiptNo}</span>
                                )}
                              </td>
                              <td className="p-3">
                              ...
                              <td className="p-3 text-right font-semibold text-rose-600">{balAfter?.toLocaleString()}</td>
                              <td className="p-3 text-center">
                                <button
                                  type="button"
                                  onClick={() => setSelectedReceiptPayment(p)}
                                  className="btn-outline btn-xs px-2 py-1 inline-flex items-center gap-1 text-slate-700 hover:text-primary hover:border-primary"
                ...
                <div className="p-4 bg-slate-50 border-t flex flex-col sm:flex-row sm:justify-end gap-3 sm:gap-8 text-sm">
                  ...
                  <div className="flex justify-between sm:block text-right"><p className="text-slate-500 text-left sm:text-right">Total Paid (All):</p><p className="font-bold text-emerald-600">{cur} {(inv?.totalPaid || 0).toLocaleString()}</p></div>
                  <div className="flex justify-between sm:block text-right"><p className="text-slate-500 text-left sm:text-right">Remaining Balance:</p><p className="font-bold text-rose-600">{cur} {(inv?.remainingBalance || 0).toLocaleString()}</p></div>
                </div>
              </div>
            </div>
          )}
