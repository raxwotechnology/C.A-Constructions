...
import { handlePayrollSyncResponse, invalidatePayrollQueries } from '../../lib/payrollSync'
...
import { FiDollarSign, FiPlay, FiCheck, FiPlus, FiSend, FiX, FiAlertCircle, FiRefreshCw, FiFileText, FiEye, FiPrinter, FiUser, FiChevronDown, FiTrash2, FiDownload } from 'react-icons/fi'
...
export default function AdminPayroll() {
  ...
  const selectedOtEmp = useMemo(() => activeEmployees.find(e => String(e._id) === String(otEmployeeId)), [activeEmployees, otEmployeeId])
  const otRate = Number(selectedOtEmp?.otRatePerHour || 0)

  const handleOtEmployeeChange = (id) => {
    setOtEmployeeId(id)
    const emp = activeEmployees.find(e => String(e._id) === String(id))
    const rate = Number(emp?.otRatePerHour || 0)
    if (rate > 0 && otHours > 0) {
      setOtAmount(otHours * rate)
    }
  }

  const handleOtHoursChange = (hVal) => {
    const h = Math.max(0, Number(hVal || 0))
    setOtHours(h)
    if (otRate > 0) {
      setOtAmount(h * otRate)
    }
  }
  ...
  const addOtMut = useMutation({
    ...
    onSuccess: (res) => { 
      ...
      handlePayrollSyncResponse(qc, res.data, toast);
      invalidatePayrollQueries(qc, otEmployeeId);
      refetch();
    },
    onError: e => toast.error(e.response?.data?.message || 'Failed'),
  })
  const deleteOtMut = useMutation({
    mutationFn: (id) => api.delete(`/payroll/overtime/${id}`),
    onSuccess: (res) => {
    ...
    onError: e => toast.error(e.response?.data?.message || 'Failed to delete OT record'),
  })
  ...
  const previewCalc = useMemo(() => {
    ...
    const safeNum = (v) => {
      if (Array.isArray(v)) {
        return v.reduce((acc, item) => acc + (Number(typeof item === 'object' && item !== null ? (item.amount || 0) : item) || 0), 0)
      }
      const n = Number(v)
      return Number.isFinite(n) ? n : 0
    }

    const currentAllowances = safeNum(allowances)
    const currentCommissions = safeNum(commissions)
    const currentBonus = safeNum(bonus)
    const currentAdvance = safeNum(advanceDeduction)
    const currentLoan = safeNum(loanDeduction)
    const currentOtherDeductions = safeNum(otherDeductions)
    const currentLeave = safeNum(leaveDeduction)

    const basic = safeNum(liveSnap.basicSalary)
    const otTotal = safeNum(liveSnap.overtime || liveSnap.otPay)
    const projectComm = safeNum(liveSnap.projectCommissions || projectCommissionTotal)
    const incentives = safeNum(liveSnap.incentives)
    ...
    const epfEmp = safeNum(liveSnap.epfEmployee)
    const epfEmpl = safeNum(liveSnap.epfEmployer)
    const etf = safeNum(liveSnap.etfEmployer)
    
    const penalty = safeNum(liveSnap.penaltyDeduction)
    const incomeTax = safeNum(liveSnap.incomeTaxDeduction || livePayrollData?.incomeTax?.taxAmount)
    ...
    return {
      ...
      totalAdvance: (activeAdvances || []).reduce((s, a) => s + safeNum(a.outstandingBalance), 0),
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="card card-body space-y-3">
          ...
          <div>
            ...
            <SearchableSelect
              value={otEmployeeId}
              onChange={(v) => handleOtEmployeeChange(v)}
              loadOptions={lookupLoaders.employees({ branch: branchFilter })}
              placeholder="Search employee…"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="form-label">OT Amount (LKR)</label>
              <input
                type="number"
                min="0"
                className="form-input"
          ...
          {otRate > 0 ? (
            <p className="text-[11px] text-blue-600 font-medium bg-blue-50 px-2.5 py-1.5 rounded border border-blue-100">
              💡 Rate: LKR {otRate.toLocaleString()}/hr {otHours > 0 ? `(${otHours} hrs = LKR ${(otHours * otRate).toLocaleString()})` : ''}
            </p>
          ) : otEmployeeId ? (
            <p className="text-[11px] text-slate-400 bg-slate-50 px-2.5 py-1.5 rounded border border-slate-100">
              ℹ️ Hourly rate not set in profile — enter custom amount above.
            </p>
          ) : null}
          ...
          <button className="btn-outline w-full" disabled={!otEmployeeId || addOtMut.isPending} onClick={() => addOtMut.mutate()}>
          ...
          <div className="mt-4 border-t border-slate-100 pt-3 space-y-2">
            ...
            {otRecords.length === 0 ? (
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {otRecords.map(ot => (
                  <div key={ot._id} className="p-2.5 rounded-lg bg-slate-50 border border-slate-100 flex justify-between items-center text-sm">
                    <div>
                      <p className="font-semibold text-slate-700">{ot.employee?.userId?.name || ot.employee?.fullName || 'Employee'}</p>
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <button
                        ...
                        disabled={deleteOtMut.isPending}
                        onClick={() => {
                          if (window.confirm(`Delete OT record for ${ot.employee?.userId?.name || ot.employee?.fullName || 'employee'} (${ot.hours} hrs)?`)) {
                            deleteOtMut.mutate(ot._id)
                          }
                        }}
                      >
                        <FiTrash2 size={15} />
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4" style={{ zIndex: 99999 }}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="flex gap-3 p-5 border-t border-gray-100 flex-wrap flex-shrink-0">
              <button onClick={() => handlePrintSlip(previewPayroll)} className="btn-outline btn-sm gap-1.5">
                <FiPrinter size={14}/> Print
              </button>
              <button onClick={() => handleDownloadSlip(previewPayroll)} className="btn-primary btn-sm gap-1.5 shadow-sm">
