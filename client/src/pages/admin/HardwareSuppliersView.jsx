...
export default function HardwareSuppliersView() {
  ...
  const [activeTab, setActiveTab] = useState('pos') // 'suppliers' | 'pos' | 'po_summary'
  ...
  // Send POs to Expenses Mutation
  const sendPOsToExpensesMutation = useMutation({
    mutationFn: async (poIds) => {
      const res = await api.post('/suppliers/pos/send-to-expenses', { poIds })
      return res.data
    },
    onSuccess: (data) => {
      toast.success(data?.message || 'Selected Purchase Orders sent to Expenses successfully!')
      setSelectedPOIds([])
      queryClient.invalidateQueries({ queryKey: ['hardware-pos'] })
      queryClient.invalidateQueries({ queryKey: ['hardware-suppliers'] })
      queryClient.invalidateQueries({ queryKey: ['supplier-ledger'] })
    ...
    onError: (err) => {
      toast.error(err?.response?.data?.message || 'Failed to send Purchase Orders to Expenses')
    }
  })
  ...
  const recordPaymentMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Payment recorded and added to Expenses!')
      queryClient.invalidateQueries({ queryKey: ['hardware-suppliers'] })
      queryClient.invalidateQueries({ queryKey: ['supplier-ledger'] })
    ...
    <div className="p-4 md:p-6 space-y-6 bg-slate-50 text-slate-800 min-h-screen">
      ...
      <div className="flex items-center justify-between gap-4 border-b border-slate-200 pb-2">
        <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-slate-200">
          ...
          <button
            onClick={() => setActiveTab('po_summary')}
            className={`px-4 py-2 rounded-lg font-bold text-xs transition-all cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'po_summary' ? 'bg-amber-500 text-slate-950 shadow-sm' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            <FiFileText size={14} />
            PO Summary ({pos.length})
          </button>
      ...
      {/* ------------------------------------------------------------- */}
      {/* TAB: PURCHASE ORDER SUMMARY & EXPENSE POSTING                */}
      {/* ------------------------------------------------------------- */}
      {activeTab === 'po_summary' && (() => {
        const totalPOAmount = pos.reduce((sum, p) => sum + (Number(p.totalAmount) || 0), 0)
        const sentPOs = pos.filter(p => Boolean(p.financeEntryRef || p.isSentToExpenses))
        const sentPOAmount = sentPOs.reduce((sum, p) => sum + (Number(p.totalAmount) || 0), 0)
        const pendingPOs = pos.filter(p => !p.financeEntryRef && !p.isSentToExpenses)
        const pendingPOAmount = pendingPOs.reduce((sum, p) => sum + (Number(p.totalAmount) || 0), 0)
        const deliveredPOs = pos.filter(p => p.status === 'Delivered')
        const deliveredPOAmount = deliveredPOs.reduce((sum, p) => sum + (Number(p.totalAmount) || 0), 0)

        const summaryPOs = sortedAndFilteredPOs
        const allPendingIdsInView = summaryPOs
          .filter(p => !p.financeEntryRef && !p.isSentToExpenses)
          .map(p => p._id)

        const toggleSelectAll = () => {
          if (selectedPOIds.length === allPendingIdsInView.length && allPendingIdsInView.length > 0) {
            setSelectedPOIds([])
          } else {
            setSelectedPOIds(allPendingIdsInView)
          }
        }
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Total Purchase Orders</p>
                  <h3 className="text-2xl font-black text-slate-900 mt-1">LKR {totalPOAmount.toLocaleString()}</h3>
                  <p className="text-xs text-slate-500 mt-1">{pos.length} total orders recorded</p>
                </div>
              ...
              <div className="bg-white rounded-2xl p-5 border border-amber-200 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-black text-amber-600 mt-1">LKR {pendingPOAmount.toLocaleString()}</h3>
                  <p className="text-xs text-slate-500 mt-1">{pendingPOs.length} POs not yet sent to finance</p>
                </div>
                <div className="w-11 h-11 rounded-xl bg-amber-500/10 text-amber-600 flex items-center justify-center shrink-0">
              ...
              <div className="bg-white rounded-2xl p-5 border border-emerald-200 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-black text-emerald-600 mt-1">LKR {sentPOAmount.toLocaleString()}</h3>
                  <p className="text-xs text-slate-500 mt-1">{sentPOs.length} POs posted to finance ledger</p>
                </div>
              ...
              <div className="bg-white rounded-2xl p-5 border border-blue-200 shadow-sm flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Delivered Orders</p>
                  <h3 className="text-2xl font-black text-blue-600 mt-1">LKR {deliveredPOAmount.toLocaleString()}</h3>
                  <p className="text-xs text-slate-500 mt-1">{deliveredPOs.length} received on site/warehouse</p>
                </div>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                <h4 className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
                  ...
                  Purchase Order Summary &amp; Expense Posting
                </h4>
                <p className="text-xs text-slate-500 mt-0.5">
                  Select pending purchase orders and click <strong>"Send to Expenses"</strong> to post them to the Finance Ledger.
                </p>
              </div>

              <div className="flex items-center gap-2.5 shrink-0">
                {selectedPOIds.length > 0 && (
                  <span className="text-xs bg-amber-100 text-amber-800 font-bold px-3 py-1.5 rounded-full border border-amber-200">
                    {selectedPOIds.length} selected
                  </span>
                )}
                <button
                  type="button"
                  disabled={selectedPOIds.length === 0 || sendPOsToExpensesMutation.isPending}
                  onClick={() => sendPOsToExpensesMutation.mutate(selectedPOIds)}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-extrabold text-xs shadow-md transition-all cursor-pointer"
                >
                  <FiCheckCircle className="w-4 h-4" />
                  {sendPOsToExpensesMutation.isPending ? 'Sending...' : 'Send to Expenses'}
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              {summaryPOs.length === 0 ? (
                <div className="p-12 text-center text-slate-500">
                  <FiShoppingBag className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                  <p className="text-sm font-bold">No purchase orders found matching your filters.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-slate-900 text-white text-[11px] uppercase tracking-wider">
                      <tr>
                        <th className="px-4 py-3 w-10 text-center">
                          <input
                            type="checkbox"
                            checked={selectedPOIds.length === allPendingIdsInView.length && allPendingIdsInView.length > 0}
                            onChange={toggleSelectAll}
                            disabled={allPendingIdsInView.length === 0}
                            className="rounded border-slate-400 accent-amber-500 cursor-pointer"
                            title="Select all pending POs"
                          />
                        </th>
                        ...
                        <th className="px-4 py-3">Items Overview</th>
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {summaryPOs.map((po, idx) => {
                        const isSent = Boolean(po.financeEntryRef || po.isSentToExpenses)
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3 text-center">
                              {!isSent ? (
                                <input
                                  type="checkbox"
                                  checked={isSelected}
                                  onChange={() => toggleSelectOne(po._id)}
                                  className="rounded border-slate-300 accent-amber-500 cursor-pointer"
                                />
                              ) : (
                                <FiCheck className="w-4 h-4 text-emerald-600 mx-auto" title="Already sent to expenses" />
                              )}
                            </td>
                            ...
                            <td className="px-4 py-3">
                              <div className="font-medium text-slate-800">{po.siteName || po.project?.name || po.project?.title || 'Central Warehouse'}</div>
                              {po.project?.code && (
                                <span className="text-[10px] text-slate-400">{po.project.code}</span>
                              )}
                            </td>

                            <td className="px-4 py-3">
                              <span className="font-semibold text-slate-900">{po.items?.length || 0} Item(s)</span>
                              <div className="text-[10px] text-slate-500 truncate max-w-[180px]" title={po.items?.map(i => `${i.itemName} (${i.quantity})`).join(', ')}>
              ...
              <div className="p-3 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-600 gap-2">
                <div>
                  Showing <span className="font-bold text-slate-900">{summaryPOs.length}</span> of{' '}
                  <span className="font-bold text-slate-900">{pos.length}</span> Purchase Orders
                </div>
                <div className="flex items-center gap-4">
                  <span>
                    Summary Total: <strong className="text-slate-900">LKR {summaryPOs.reduce((s, p) => s + (Number(p.totalAmount) || 0), 0).toLocaleString()}</strong>
                  </span>
                  <span>
                    Pending to Send: <strong className="text-amber-600">{allPendingIdsInView.length}</strong>
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 w-full max-w-3xl rounded-2xl shadow-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-3 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
              <div>
                ...
                <p className="text-base font-bold text-slate-900">LKR {(ledgerData?.supplier?.totalBilled ?? selectedSupplier.totalBilled ?? 0).toLocaleString()}</p>
              </div>
              <div>
                <span className="text-xs text-slate-500">Total Paid</span>
                <p className="text-base font-bold text-emerald-600">LKR {(ledgerData?.supplier?.totalPaid ?? selectedSupplier.totalPaid ?? 0).toLocaleString()}</p>
              </div>
              <div>
                <span className="text-xs text-slate-500">Net Outstanding</span>
                <p className="text-base font-bold text-amber-600">LKR {(ledgerData?.supplier?.outstandingBalance ?? selectedSupplier.outstandingBalance ?? 0).toLocaleString()}</p>
              </div>
            </div>
