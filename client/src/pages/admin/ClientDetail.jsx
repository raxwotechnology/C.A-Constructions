...
import { FiX, FiMail, FiPhone, FiFolder, FiFileText, FiCreditCard, FiServer, FiEdit2, FiPlus, FiCheck, FiPrinter, FiDollarSign } from 'react-icons/fi'
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
          ...
          {activeTab === 'invoices' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-2">
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <p className="text-xl font-black text-slate-900">
                    LKR {(invoices || []).reduce((s, i) => s + (Number(i.total) || 0), 0).toLocaleString()}
                  </p>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Paid</p>
                  <p className="text-xl font-black text-emerald-600">
                    LKR {(invoices || []).reduce((s, i) => s + (Number(i.totalPaid) || 0), 0).toLocaleString()}
                  </p>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Balance Due</p>
                  <p className="text-xl font-black text-rose-600">
                    LKR {(invoices || []).reduce((s, i) => s + (Number(i.remainingBalance) || 0), 0).toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              <div className="table-container bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                <table className="table">
                  ...
                  <tbody>
                    {invoices.map(i => (
                      <tr key={i._id}>
                        <td className="font-semibold text-slate-800">{i.invoiceNo}</td>
                        <td className="text-slate-500 text-sm">{i.dueDate ? new Date(i.dueDate).toLocaleDateString('en-LK') : '—'}</td>
                        <td className="font-semibold text-right">LKR {Number(i.total || 0).toLocaleString()}</td>
                        <td className="font-semibold text-right text-emerald-600">LKR {Number(i.totalPaid || 0).toLocaleString()}</td>
                        <td className={`font-bold text-right ${i.remainingBalance > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                          LKR {Number(i.remainingBalance || 0).toLocaleString()}
                        </td>
                        <td>
                          <span className={`badge ${i.status === 'paid' ? 'badge-green' : i.status === 'overdue' ? 'badge-red' : i.status === 'partial' ? 'badge-blue' : 'badge-yellow'} capitalize`}>
                            {i.status}
                          </span>
                        </td>
                      </tr>
                    ))}
                    {invoices.length === 0 && <tr><td colSpan={6} className="text-center py-4 text-slate-400">No invoices.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
                ...
                {subscriptions.map(s => (
                  <div key={s._id} className="card card-body border border-slate-200">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-slate-800 truncate">{s.serviceName || s.title}</h4>
                      <span className={`badge ${s.status === 'active' ? 'badge-green' : 'badge-red'} capitalize`}>{s.status}</span>
                    </div>
                    <p className="text-sm font-medium text-secondary mb-3">LKR {Number(s.amount || 0).toLocaleString()}</p>
                    <p className="text-xs text-slate-400">Next billing: {s.nextDueDate || s.nextBillingDate ? new Date(s.nextDueDate || s.nextBillingDate).toLocaleDateString() : '—'}</p>
                  </div>
                ))}
          ...
          {activeTab === 'payments' && (
            <div className="space-y-4">
              {(() => {
                ...
                allInvoicePayments.sort((a, b) => new Date(b.date) - new Date(a.date));

                return (
                  <>
                    <div className="flex justify-between items-center">
                      <h3 className="font-bold text-lg text-primary">
                        Payment &amp; Installment History ({allInvoicePayments.length || payments.length})
                      </h3>
                    </div>
                    ...
                    {allInvoicePayments.length > 0 ? (
                      <div className="table-container bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                        <table className="table">
                          ...
                          <tbody>
                            {allInvoicePayments.map((p, idx) => (
                              <tr key={p._id || idx}>
                                <td className="text-slate-700">
                                  <span className="font-semibold">{new Date(p.date || p.createdAt).toLocaleDateString('en-LK')}</span>
                                  {p.receiptNo && <span className="block text-[11px] font-mono text-secondary">{p.receiptNo}</span>}
                                </td>
                                <td className="font-medium text-slate-800">{p.invoiceNo || '—'}</td>
                                <td>
                                  {p.isAdvance ? <span className="badge badge-purple">Advance</span> : <span className="badge badge-blue">Installment</span>}
                                </td>
                                <td className="text-slate-600 capitalize">
                                  {(p.method || 'cash').replace('_', ' ')}
                                  {p.reference && <span className="text-slate-400 text-xs font-mono block">{p.reference}</span>}
                                </td>
                                <td className="font-bold text-right text-emerald-600">
                                  LKR {Number(p.amount || 0).toLocaleString()}
                                </td>
                                <td className="font-semibold text-right text-rose-600">
                                  LKR {Number(p.remainingBalanceAfter || 0).toLocaleString()}
                                </td>
                                <td className="text-center">
                                  <button
                                    type="button"
                                    onClick={() => setSelectedReceipt({ payment: p, invoice: p.invoice })}
                          ...
                          <tbody>
                            {payments.map(p => (
                              <tr key={p._id}>
                                ...
                                <td className="font-medium text-emerald-600">{p.currency || 'LKR'} {Number(p.amount || 0).toLocaleString()}</td>
