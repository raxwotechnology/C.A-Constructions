...
export default function AdminAttendance() {
  ...
  const handleSubmit = () => {
    const effectiveHalfDay = Boolean(form.isHalfDay || form.status === 'half_day');
    const effectiveStatus = effectiveHalfDay ? 'half_day' : form.status;
    const payload = {
      ...
      isHalfDay: effectiveHalfDay,
      isFullDay: !effectiveHalfDay && Boolean(form.isFullDay),
      notes: form.notes,
    }
    if (form.checkIn) {
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <div>
                  ...
                  <select className="form-select" value={form.status} onChange={e => {
                    ...
                    setForm(s => ({
                      ...s,
                      status: st,
                      isHalfDay: st === 'half_day' ? true : (st === 'present' && s.isHalfDay ? false : s.isHalfDay)
                    }));
                  }}>
                    {STATUS_OPTIONS.map(s => <option key={s} value={s} className="capitalize">{s.replace('_', ' ')}</option>)}
                  </select>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={form.isHalfDay || form.status === 'half_day'} onChange={e => {
                    const checked = e.target.checked;
                    setForm(s => ({
                      ...s,
                      isHalfDay: checked,
                      status: checked ? 'half_day' : (s.status === 'half_day' ? 'present' : s.status)
                    }));
                  }} className="rounded" />
                  Half Day
                </label>
