...
const now = new Date()
const EMPTY = { date: new Date().toISOString().split('T')[0], type:'expense', category:'', title:'', amount:0, note:'', voucherNumber:'', file:null, branch:'', paymentMethod:'Bank Transfer', bankAccount:'', project:'' }
...
export default function FinanceEntries() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['finance-entries', month, year, typeFilter, categoryFilter, fromDate, toDate, branchFilter, paymentFilter, projectFilter],
    queryFn: () => {
    ...
    <div className="erp-module space-y-5 animate-fade-in">
      ...
      <div className="card card-body filter-toolbar space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
          ...
          <div>
            <label className="form-label text-xs">Project Site</label>
            <select className="form-select text-sm py-2 bg-amber-50/60 font-semibold border-amber-200" value={projectFilter} onChange={e=>setProjectFilter(e.target.value)}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3 px-0.5">
          ...
          <span className="text-xs text-slate-400 font-mono">{entries.length} records</span>
        </div>
                  ...
                  <div className="p-4 pl-5 flex flex-col sm:flex-row justify-between gap-4">
                    <div className="flex-1 space-y-2 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {e.voucherNumber ? (
                          <span className="badge text-[10px] font-mono font-bold bg-amber-50 text-amber-800 border border-amber-200">
                      ...
                      {e.bankAccount || e.branch || e.project ? (
                        <div className="flex flex-wrap gap-2 mt-1">
                          {e.project ? (
                            <span className="inline-flex items-center text-[11px] bg-amber-50 text-amber-900 border border-amber-200 font-bold px-2 py-0.5 rounded-md max-w-[240px] truncate" title={e.project.name || e.project.title}>
                        ...
                        <p className="text-[11px] text-slate-400 mt-0.5">No bank, branch or project linked</p>
                      )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-0 sm:p-4 z-[99999]">
          <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} className="bg-white rounded-none sm:rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[100dvh] sm:max-h-[90vh]">
            ...
            <div className="p-5 space-y-4 overflow-y-auto min-h-0 flex-1">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="form-select font-semibold text-amber-900 bg-amber-50/50 border-amber-200" value={form.project} onChange={e=>f('project',e.target.value)}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => f('voucherNumber', e.target.value)}
                    placeholder="e.g. PV-001 / VCH-101"
                  />
                </div>
                <div>
            ...
            <div className="flex gap-3 p-5 border-t shrink-0 bg-slate-50 pb-safe">
              ...
              <button disabled={addMut.isPending || updateMut.isPending} onClick={()=>{
                const cat = finalCategory || activeCategories[0] || 'General';
                const tit = form.title.trim() || cat;
                const amt = Number(form.amount || 0);
                if (!amt) return toast.error('Please enter a valid amount');
                const payload = {...form, category: cat, title: tit, amount: amt}
          ...
          <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-6 space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                ...
                {viewingEntry.voucherNumber ? (
                  <div className="col-span-2 bg-amber-50/70 border border-amber-200/80 rounded-lg p-2.5 flex items-center justify-between">
                    <span className="text-amber-800 text-xs font-bold uppercase tracking-wide">Voucher Number</span>
                    <span className="font-mono font-bold text-amber-950 text-sm">{viewingEntry.voucherNumber}</span>
                  </div>
                ) : null}
