...
import { FiPlus, FiX, FiFolder, FiSearch, FiEdit2, FiTrash2, FiUsers, FiInfo, FiUser, FiFileText, FiDollarSign } from 'react-icons/fi'
...
export default function AdminProjects() {
  ...
  const projects = (projData?.projects || []).filter(p =>
    ...
    (!priorityFilter || String(p.priority || 'medium').toLowerCase() === priorityFilter.toLowerCase())
  )
  ...
  const onSubmit = d => {
    ...
    targetId ? updateMut.mutate({ id: targetId, data: payload }) : createMut.mutate(payload)
  }
          ...
          <motion.div key={p._id} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="card card-body card-hover">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button
                ...
                className="py-2 px-2.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-900 border border-amber-300/50 font-bold text-xs rounded-xl transition-colors flex justify-center items-center gap-1 shadow-2xs cursor-pointer"
                title="Generate New Invoice for this Project"
              >
                <FiFileText size={13} className="text-amber-600"/> Generate Invoice
              </button>

              <button
                onClick={() => navigate(`/admin/projects/${p._id}`)}
                className="py-2 px-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-colors border border-slate-200 flex justify-center items-center gap-1 cursor-pointer"
              >
                <FiInfo size={13}/> View Details
              </button>
            </div>
          </motion.div>
        ))}
