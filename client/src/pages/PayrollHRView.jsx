...
export default function PayrollHRView() {
  ...
  const { data: epfData, isLoading } = useQuery({
    queryKey: ['epf-summary-hr-view', month, year],
    queryFn: () => api.get(`/payroll/epf-summary?month=${month}&year=${year}`).then(r => r.data),
  });

  const handleExportFormC = () => {
    const summary = epfData?.summary || [];
    if (summary.length === 0) {
      toast.error('No EPF records found for export. Please process payroll first.');
      return;
    }
    exportEpfFormC(summary, `${year}-${String(month).padStart(2, '0')}`);
  };
      ...
      {activeTab === 'compliance' && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <button 
            ...
            disabled={isLoading}
            className="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white font-semibold px-4 py-2.5 rounded-xl text-sm shadow-sm cursor-pointer transition-all"
          >
