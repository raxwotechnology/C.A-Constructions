...
import { FiDollarSign, FiDownload, FiTrendingUp, FiPrinter } from 'react-icons/fi'
            ...
            <div key={p._id} className="card card-body card-hover">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div className="flex gap-2 flex-shrink-0 items-center">
                  <button
                    onClick={() => downloadPayslipPdf(p, siteSettings || {}, { role: p.payslipSignatory?.role, customSignatureUrl: p.payslipSignatory?.signatureUrl })}
                    className="btn-primary btn-sm flex items-center gap-1.5 font-semibold shadow-sm"
                  >
                    <FiDownload size={13}/> Download PDF
                  </button>
                  <button
                    onClick={() => printPayslipDoc(p, siteSettings || {}, { role: p.payslipSignatory?.role, customSignatureUrl: p.payslipSignatory?.signatureUrl })}
