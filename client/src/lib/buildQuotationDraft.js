...
export function buildQuotationDraft(form, { clients = [], editing = null, user } = {}) {
  ...
  const labelClean = (form?.clientSelectLabel || form?.clientLabel || '').replace(/\s*\([^)]*\)/, '').trim()
  const fallbackName = form?.customerName || form?.clientName || labelClean || (typeof form?.client === 'object' ? form?.client?.name : '')
