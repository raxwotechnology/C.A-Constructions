...
async function seed() {
  ...
  // Ensure employee profiles exist for PM and Supervisor
  let empPM = await Employee.findOne({ userId: userMap.manager._id });
  if (!empPM) {
    empPM = await Employee.create({
      userId: userMap.manager._id,
      employeeNo: 'EMP-1002',
  ...
  let empSup = await Employee.findOne({ userId: userMap.supervisor._id });
  if (!empSup) {
    empSup = await Employee.create({
      userId: userMap.supervisor._id,
      employeeNo: 'EMP-1003',
  ...
  await WorkLog.create([
    {
      ...
      date: new Date(),
      tasks: [
        { taskName: 'SLS 573 BOQ Review & Inspection for Lotus Villa', hours: 4, project: sitesList[0]._id, notes: 'Checked beam reinforcement' },
        { taskName: 'Client Progress Meeting & Milestone Review', hours: 3, project: sitesList[0]._id, notes: 'Approved Phase 1 payments' }
      ],
      totalHours: 7,
      status: 'submitted',
    ...
    },
    {
      ...
      date: new Date(),
      tasks: [
        { taskName: 'Supervised Grade 30 Concrete Pouring', hours: 5, project: sitesList[0]._id, notes: 'Inspected 18m3 poured' },
        { taskName: 'Safety Toolbox Talk & Helmet Check', hours: 2, project: sitesList[0]._id, notes: 'Zero safety incidents' }
      ],
      totalHours: 7,
      status: 'submitted',
      approvalStatus: 'pending',
      blockers: 'Slight morning rain delay',
      notes: 'All workers present for morning toolbox talk.'
    }
  ]);
  console.log('✓ 2 WorkLog Daily Submissions Seeded');
