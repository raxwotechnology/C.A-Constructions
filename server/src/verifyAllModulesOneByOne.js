...
check('Asset Categories (Master Categories)', () => {
  if (ASSET_CATEGORIES.length < 7) throw new Error('Expected at least 7 asset categories');
});
check('Liability Categories (Master Categories)', () => {
  if (LIABILITY_CATEGORIES.length < 4) throw new Error('Expected at least 4 liability categories');
});
check('Capital Categories (Master Categories)', () => {
  if (CAPITAL_CATEGORIES.length < 3) throw new Error('Expected at least 3 capital categories');
});
check('Tax Categories (VAT 18%, APIT, Income Tax, Stamp Duty)', () => {
  if (TAX_CATEGORIES.length < 4) throw new Error('Expected at least 4 tax categories');
});
