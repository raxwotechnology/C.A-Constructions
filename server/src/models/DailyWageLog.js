...
const dailyWageLogSchema = new mongoose.Schema(
  {
    ...
    advanceFinanceEntryRef: { type: mongoose.Schema.Types.ObjectId, ref: 'FinanceEntry' },
...
dailyWageLogSchema.pre('validate', function (next) {
  if (this.workType === 'Daily Wage') {
    ...
    let totalMeasured = 0;
    if (isLumpSum) {
      totalMeasured = Number(this.subContractDetails?.lumpSumAmount || this.subContractDetails?.totalMeasuredPay || 0);
    } else {
      const sqft = this.subContractDetails?.measuredSqft || 0;
      const rate = this.subContractDetails?.ratePerSqft || 0;
      totalMeasured = (sqft * rate) || Number(this.subContractDetails?.totalMeasuredPay || 0);
    }
