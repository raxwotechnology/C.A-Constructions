...
exports.getPayrolls = async (req, res, next) => {
  try {
    ...
    if (month && year) {
      ...
      if (month) {
        const mNum = Number(month);
        query.$or = [{ month: mNum }, { month: String(mNum) }];
      }
...
exports.updatePayroll = async (req, res, next) => {
  try {
    ...
    const parseArrayOrNum = (val) => {
      if (Array.isArray(val)) {
        return val.reduce((s, i) => s + (Number(typeof i === 'object' && i !== null ? (i.amount || 0) : i) || 0), 0);
      }
      return Number(val || 0);
    };

    const addOthers = parseArrayOrNum(otherAdditions !== undefined ? otherAdditions : existing.otherAdditions);
    const dedOthers = parseArrayOrNum(otherDeductions !== undefined ? otherDeductions : existing.otherDeductions);
    ...
    const updated = await Payroll.findByIdAndUpdate(
      req.params.id,
      {
        ...
        otherAdditions: addOthers,
        otherDeductions: dedOthers,
...
exports.getEpfSummary = async (req, res, next) => {
  try {
    ...
    const enrolledEmployees = await Employee.find({
      $or: [
        { epfEtfEnrolled: true },
        { isEpfEligible: { $ne: false } },
        { epfNumber: { $exists: true, $ne: '' } },
        { epfEtfEnrolled: { $exists: false } }
      ],
      status: { $nin: ['former', 'terminated', 'intern_ended', 'Inactive', 'inactive'] },
    ...
    const summary = enrolledEmployees.map(emp => {
      ...
      return {
        ...
        employeeNo: emp.employeeNo || emp.employeeId,
        name:       emp.userId?.name || emp.fullName || 'Employee',
        epfNo:      emp.epfNumber || '—',
        etfNo:      emp.etfNumber || '—',
        basicSalary,
        epfEmployee,
