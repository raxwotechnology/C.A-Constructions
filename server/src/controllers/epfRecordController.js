...
exports.getEpfRecords = async (req, res, next) => {
  try {
    ...
    // All active enrolled employees (enrolled if true, or isEpfEligible not false, or epfNumber exists)
    ...
    if (toCreate.length) {
      await EpfRecord.insertMany(toCreate);
    }
    ...
    const summary = records.filter(rec => rec.employee).map(rec => ({
      _id:         rec._id,
      employeeId:  rec.employee._id,
      employeeNo:  rec.employee.employeeNo || rec.employee.employeeId,
      name:        rec.employee.userId?.name || rec.employee.fullName || 'Employee',
      epfNo:       rec.employee.epfNumber || '—',
      etfNo:       rec.employee.etfNumber || '—',
      basicSalary: rec.basicSalary || 0,
      epfEmployee: rec.epfEmployee || 0,
      epfEmployer: rec.epfEmployer || 0,
      totalEPF:    rec.totalEPF || ((rec.epfEmployee || 0) + (rec.epfEmployer || 0)),
      etfEmployer: rec.etfEmployer || 0,
      isPaid:      rec.isPaid,
      paidAt:      rec.paidAt,
      notes:       rec.notes,
    }));
