...
exports.createClient = async (req, res, next) => {
  try {
    ...
    const cleanPassword = password ? String(password).trim() : 'Client@2026';

    // Check if client already exists by email or phone/name
    if (cleanEmail) {
      const existing = await User.findOne({ email: cleanEmail });
      if (existing) {
        if (existing.role === 'client') {
          return res.status(200).json({ success: true, user: existing, message: 'Client already exists' });
        }
        return res.status(400).json({ success: false, message: 'Email already registered with another account role' });
      }
    } else if (cleanPhone) {
      const existingPhone = await User.findOne({ phone: cleanPhone, role: 'client' });
      if (existingPhone) {
        return res.status(200).json({ success: true, user: existingPhone, message: 'Client already exists' });
      }
    }
