...
const POPULATE = [
  ...
  {
    path: 'invoice',
    select: 'invoiceNo invoiceDate dueDate total totalPaid totalAdvances remainingBalance status currency items payments notes',
...
exports.getProjects = async (req, res, next) => {
  try {
    ...
    const projects = await Project.find(query)
      ...
      .populate({
        path: 'invoice',
        select: 'invoiceNo total totalPaid remainingBalance status payments',
      })
      .sort({ createdAt: -1 });
    ...
    const enrichedProjects = await Promise.all(
      projects.map(async (p) => {
        ...
        // Payments & Billing tracking
        const invTotal = Number(p.invoice?.total || 0);
        const invPaid = Number(p.invoice?.totalPaid || 0);
        pObj.totalBilled = invTotal || p.contractValue || 0;
        pObj.totalCollected = invPaid || p.advancePaid || 0;
        pObj.remainingBalance = Math.max(0, pObj.totalBilled - pObj.totalCollected);
...
exports.getProject = async (req, res, next) => {
  try {
    ...
    // Collect all linked invoice payments
    const allInvoices = [
      ...(pObj.invoice ? [pObj.invoice] : []),
      ...(pObj.linkedInvoices || []),
    ];
    // Deduplicate by ID
    ...
    pObj.totalBilled = sumBilled > 0 ? sumBilled : (pObj.contractValue || 0);
    pObj.totalCollected = sumPaid > 0 ? sumPaid : (pObj.advancePaid || 0);
    pObj.remainingBalance = Math.max(0, pObj.totalBilled - pObj.totalCollected);
    pObj.allPayments = allPayments;
...
function sanitizeProjectPayload(body) {
  const payload = { ...body };
  ['client', 'projectManager', 'siteSupervisor', 'branch', 'invoice'].forEach((f) => {
    if (!payload[f] || payload[f] === '' || payload[f] === 'null' || payload[f] === 'undefined') {
      delete payload[f];
    }
  });
  ['startDate', 'deadline', 'expectedCompletionDate', 'actualCompletionDate'].forEach((f) => {
    if (payload[f] === '' || payload[f] === null || payload[f] === undefined || payload[f] === 'null' || payload[f] === 'undefined') {
      delete payload[f];
    } else {
  ...
  if (Array.isArray(payload.assignedEmployees)) {
    payload.assignedEmployees = payload.assignedEmployees.filter(Boolean);
  } else if (payload.assignedEmployees !== undefined) {
