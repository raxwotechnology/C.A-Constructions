...
// Helper to resolve or auto-create client
async function resolveOrCreateClient(rawClient, extra = {}) {
  if (!rawClient || !String(rawClient).trim()) return null;
  const clientStr = String(rawClient).trim();

  if (mongoose.Types.ObjectId.isValid(clientStr)) {
    const foundById = await User.findById(clientStr);
    if (foundById) return foundById._id;
  }

  // Search by exact name (case-insensitive) or email
  let found = await User.findOne({
    $or: [
      { name: { $regex: new RegExp(`^${clientStr.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') } },
      { email: clientStr.toLowerCase() }
    ]
  });

  if (found) return found._id;

  // Auto-create new Client User
  const randomSuffix = Math.floor(1000 + Math.random() * 9000);
  const slug = clientStr.toLowerCase().replace(/[^a-z0-9]/g, '');
  const autoEmail = (extra.clientEmail && String(extra.clientEmail).trim().toLowerCase()) 
    || `${slug || 'client'}.${randomSuffix}@client.local`;
  const autoPassword = Math.random().toString(36).slice(-8) + 'Aa1!';
  
  found = await User.create({
    name: clientStr,
    email: autoEmail,
    phone: extra.clientPhone || '',
    address: extra.clientAddress || '',
    password: autoPassword,
    role: 'client',
    isActive: true,
    branch: extra.branch && mongoose.Types.ObjectId.isValid(extra.branch) ? extra.branch : undefined,
  });
...
exports.createInvoice = async (req, res, next) => {
  try {
    ...
    // Resolve client ObjectId (or auto-create if new name)
    const resolvedClientId = await resolveOrCreateClient(client, {
    ...
    const safeBranch = branch && mongoose.Types.ObjectId.isValid(branch) ? branch : (quotationDoc && quotationDoc.branch);
    const safeProject = project && mongoose.Types.ObjectId.isValid(project) ? project : undefined;
    const safeBankAccount = req.body.bankAccount && mongoose.Types.ObjectId.isValid(req.body.bankAccount) ? req.body.bankAccount : (quotationDoc && quotationDoc.bankAccount);
    ...
    const invoicePayload = {
      ...
      bankAccount: safeBankAccount || undefined,
...
exports.updateInvoice = async (req, res, next) => {
  try {
    ...
    let resolvedClientId = existing.client;
    if (client !== undefined) {
      resolvedClientId = await resolveOrCreateClient(client, {
        clientEmail: req.body.clientEmail,
        clientPhone: req.body.clientPhone,
        clientAddress: req.body.clientAddress,
        branch,
    ...
    const safeBranch = branch !== undefined ? (branch && mongoose.Types.ObjectId.isValid(branch) ? branch : null) : existing.branch;
    const safeProject = project !== undefined ? (project && mongoose.Types.ObjectId.isValid(project) ? project : null) : existing.project;
    const safeBankAccount = req.body.bankAccount !== undefined ? (req.body.bankAccount && mongoose.Types.ObjectId.isValid(req.body.bankAccount) ? req.body.bankAccount : null) : existing.bankAccount;
    const safeQuotationRef = quotationRef !== undefined ? (quotationRef && mongoose.Types.ObjectId.isValid(quotationRef) ? quotationRef : null) : existing.quotationRef;
    ...
    let updates = {
      ...
      client: resolvedClientId || existing.client,
      project: safeProject,
      branch: safeBranch,
      quotationRef: safeQuotationRef,
    ...
    if (items) {
      ...
      const total = parseFloat((baseTotal + transportCharge).toFixed(2));
      const remainingBalance = Math.max(0, total - (existing.totalPaid || 0));
      updates = { ...updates, items: calcedItems, subtotal, globalDiscountType, globalDiscountValue: globalDiscountVal, discountTotal, tax, taxRate: Number(taxRate ?? existing.taxRate), transportCharge, total, remainingBalance };
      if (status === undefined && existing.status !== 'draft' && existing.status !== 'cancelled') {
        if (remainingBalance === 0 && (existing.totalPaid || 0) > 0) updates.status = 'paid';
        else if ((existing.totalPaid || 0) > 0) updates.status = 'partial';
...
// @desc    Download / view single payment receipt PDF/HTML
// @route   GET /api/invoices/:id/payments/:paymentId/receipt
exports.downloadPaymentReceiptPdf = async (req, res, next) => {
  try {
    ...
    const payment = invoice.payments.find(p => String(p._id) === String(paymentId) || String(p.receiptNo) === String(paymentId));
    if (!payment) return res.status(404).json({ success: false, message: 'Payment entry not found' });

    const { buildPaymentReceiptHtml, inlineUploadImagesInHtml } = require('../services/documentHtmlService');
    const html = await buildPaymentReceiptHtml(invoice, payment);
    const inlinedHtml = inlineUploadImagesInHtml(html);
    ...
    const { htmlToPdfBuffer } = require('../services/documentPdfService');
    const pdf = await htmlToPdfBuffer(inlinedHtml);
    const recFilename = (payment.receiptNo || `Receipt_${payment._id}`).replace(/[^a-zA-Z0-9_-]/g, '_');
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${recFilename}.pdf"`);
    res.send(pdf);
  } catch (err) { next(err); }
};
