  ...
  try {
    ...
    switch (type) {
      case 'employees': {
        ...
        const deduped = sliceRaw.filter((e) => {
          const eid = String(e._id);
          if (seenIds.has(eid)) return false;
          seenIds.add(eid);
          return true;
        });
        options = toOptions(deduped, '_id', e => `${e.fullName || e.userId?.name || 'Worker'} (${e.employeeNo || e.employeeId || '—'})`);
        break;
      }
      case 'clients': {
        ...
        hasMore = rows.length > l;
        options = toOptions(hasMore ? rows.slice(0, l) : rows, '_id', u => {
          const detail = u.email ? ` (${u.email})` : (u.phone ? ` (${u.phone})` : '');
