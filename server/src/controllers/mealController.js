...
exports.updateVendor = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, contactPerson, phone, email, address, defaultMealRate, bankDetails, notes, status } = req.body;

    const vendor = await CateringVendor.findById(id);
    if (!vendor) {
      return res.status(404).json({ success: false, message: 'Catering Vendor not found' });
    }
    ...
    if (defaultMealRate !== undefined) vendor.defaultMealRate = Number(defaultMealRate);
    ...
    res.json({ success: true, message: 'Catering Vendor updated successfully', vendor });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.deleteVendor = async (req, res) => {
  try {
    const { id } = req.params;
    const vendor = await CateringVendor.findByIdAndDelete(id);
    if (!vendor) {
      return res.status(404).json({ success: false, message: 'Catering Vendor not found' });
    }
    res.json({ success: true, message: 'Catering Vendor deleted successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.getMealEntries = async (req, res) => {
  try {
    ...
    const entries = await MealEntry.find(filter)
      .populate('vendor', 'name phone defaultMealRate')
...
// Update meal entry
exports.updateMealEntry = async (req, res) => {
  try {
    const { id } = req.params;
    const { vendorId, date, shift, mealCount, unitPrice, projectId, notes, settlementStatus } = req.body;

    const entry = await MealEntry.findById(id);
    if (!entry) return res.status(404).json({ success: false, message: 'Meal Entry not found' });

    const oldTotalCost = entry.totalCost || 0;
    const oldVendorId = String(entry.vendor);

    const count = Number(mealCount !== undefined ? mealCount : entry.mealCount);
    const rate = Number(unitPrice !== undefined ? unitPrice : entry.unitPrice);
    const newTotalCost = count * rate;

    const newVendorId = vendorId ? String(vendorId) : oldVendorId;

    let siteName = entry.siteName;
    if (projectId !== undefined) {
      if (projectId) {
        const proj = await Project.findById(projectId);
        siteName = proj ? (proj.name || proj.title) : 'Head Office / Site';
      } else {
        siteName = 'Head Office / Site';
      }
      entry.project = projectId || null;
    ...
    entry.totalCost = newTotalCost;
    if (notes !== undefined) entry.notes = notes;
    if (settlementStatus) entry.settlementStatus = settlementStatus;
    entry.vendor = newVendorId;
    ...
    if (oldVendorId === newVendorId) {
      ...
      if (diff !== 0) {
        const vendor = await CateringVendor.findById(newVendorId);
        if (vendor) {
          vendor.outstandingBalance = Math.max(0, (vendor.outstandingBalance || 0) + diff);
          vendor.totalBilled = Math.max(0, (vendor.totalBilled || 0) + diff);
          await vendor.save();
        }
      }
    } else {
      // Deduct from old vendor
      const oldVendor = await CateringVendor.findById(oldVendorId);
      if (oldVendor) {
        oldVendor.outstandingBalance = Math.max(0, (oldVendor.outstandingBalance || 0) - oldTotalCost);
        oldVendor.totalBilled = Math.max(0, (oldVendor.totalBilled || 0) - oldTotalCost);
        await oldVendor.save();
      }
      // Add to new vendor
      const newVendor = await CateringVendor.findById(newVendorId);
      if (newVendor) {
        newVendor.outstandingBalance = (newVendor.outstandingBalance || 0) + newTotalCost;
        newVendor.totalBilled = (newVendor.totalBilled || 0) + newTotalCost;
        await newVendor.save();
      }
    }

    res.json({ success: true, message: 'Meal Entry updated successfully', entry });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.deleteMealEntry = async (req, res) => {
  try {
    const { id } = req.params;
    const entry = await MealEntry.findByIdAndDelete(id);
    if (!entry) return res.status(404).json({ success: false, message: 'Meal Entry not found' });

    // Deduct from vendor balance if unsettled
    if (entry.vendor && entry.totalCost) {
      const vendor = await CateringVendor.findById(entry.vendor);
      if (vendor) {
        vendor.outstandingBalance = Math.max(0, (vendor.outstandingBalance || 0) - entry.totalCost);
        vendor.totalBilled = Math.max(0, (vendor.totalBilled || 0) - entry.totalCost);
        await vendor.save();
      }
    }

    res.json({ success: true, message: 'Meal Entry deleted successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
