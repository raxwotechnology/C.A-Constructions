...
exports.addEntry = async (req, res, next) => {
  try {
    const { type, category, title, amount, date, note, branch, paymentMethod, transactionType, masterCategory, payeeOrPayer, transactionNo, project, voucherNumber } = req.body;
    ...
    const data = {
      ...
      transactionType: transactionType || (entryType === 'income' ? 'Income' : 'Expense'),
      masterCategory: entryCategory,
      payeeOrPayer: payeeOrPayer || entryTitle || 'General',
    };
    if (req.file) {
...
exports.updateEntry = async (req, res, next) => {
  try {
    ...
    const { type, category, title, amount, date, note, branch, paymentMethod, bankAccount, project, voucherNumber } = req.body;
    ...
    if (voucherNumber !== undefined) entry.voucherNumber = String(voucherNumber).trim();
...
exports.getEntries = async (req, res, next) => {
  try {
    const { type, category, month, year, from, to, branch, paymentMethod, project } = req.query;
    const q = {};
    if (type) q.type = type;
    ...
    if (project) {
      allEntries = allEntries.filter((e) => {
        ...
        return pId && String(pId) === String(project);
      });
    }
...
exports.getOverview = async (req, res, next) => {
  try {
    ...
    const [paidInvoices, paidPayrolls, rawEntries, revenueByMonth, paymentRevenueAgg, incomeExpenseByCategory, pettyCashEntries, subIncome, invIncome, chequeTx, advanceExp, loanTx, bankLedger, cashAgg, allAssets] = await Promise.all([
    ...
    const totalFleetAssetValue = (allAssets || []).reduce((s, a) => s + Number(a.assetValue || a.amount || 0), 0);
    const fleetAssetCount = (allAssets || []).length;
