...
exports.getClientProfile = async (req, res, next) => {
  try {
    ...
    const [projects, invoices, subscriptions, payments, agreements] = await Promise.all([
      Project.find({ client: userId }).select('title status progress budget invoice contractValue totalBilled totalCollected remainingBalance'),
      Invoice.find({ client: userId }).select('invoiceNo total totalPaid remainingBalance status dueDate payments createdAt currency notes').sort({ createdAt: -1 }),
      Subscription.find({ client: userId }).select('title subscriptionType status amount nextDueDate'),
      Payment.find({ client: userId }).select('amount currency status createdAt method reference invoice'),
