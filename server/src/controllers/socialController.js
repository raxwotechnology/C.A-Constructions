...
exports.getSocialAnalytics = async (req, res, next) => {
  try {
    ...
    const [fbRes, igRes, ytRes, tiktokRes, linkedinRes] = await Promise.allSettled([
      // Facebook
      axios.get(`https://graph.facebook.com/v18.0/${FB_PAGE_ID}?fields=followers_count,fan_count&access_token=${FB_ACCESS_TOKEN}`, { timeout: 8000 }),
      
      // Instagram
      axios.get(`https://graph.facebook.com/v18.0/${IG_BUSINESS_ACCOUNT_ID}?fields=followers_count,media_count&access_token=${IG_ACCESS_TOKEN}`, { timeout: 8000 }),

      // YouTube
      axios.get(`https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${YT_CHANNEL_ID}&key=${YT_API_KEY}`, { timeout: 8000 }),
