...
const { addEntry, getEntries, getOverview, exportData, getProfitLoss, updateEntry, deleteEntry, syncSubscriptionIncome, getSitePnlSummary } = require('../controllers/financeController');

router.get('/overview', protect, authorize('admin'), getOverview);
router.get('/entries', protect, authorize('admin'), getEntries);
router.get('/site-summary', protect, authorize('admin'), getSitePnlSummary);
