...
function derivePaymentStatus(invoice) {
  ...
  const remaining = typeof invoice.remainingBalance === 'number' && invoice.remainingBalance > 0
...
async function applyPaymentStatusToProject(project, invoices) {
  const validInvoices = (invoices || []).filter((inv) => inv && inv.status !== 'cancelled');
  const paymentStatus = deriveAggregatePaymentStatus(validInvoices);
  project.paymentStatus = paymentStatus;

  // Auto-aggregate financial collections
  const totalBilled = validInvoices.reduce((sum, inv) => sum + (Number(inv.total) || 0), 0);
  const totalCollected = validInvoices.reduce((sum, inv) => sum + (Number(inv.totalPaid) || 0), 0);
  const totalAdvances = validInvoices.reduce((sum, inv) => sum + (Number(inv.totalAdvances) || 0), 0);

  if (totalBilled > 0) project.totalBilled = totalBilled;
  project.totalCollected = totalCollected;
  project.totalIncome = totalCollected;
  if (totalAdvances > 0) project.advancePaid = totalAdvances;
...
async function syncProjectsForInvoice(invoiceId) {
  ...
  const projectIds = [];
  if (invoice.project) projectIds.push(invoice.project);
