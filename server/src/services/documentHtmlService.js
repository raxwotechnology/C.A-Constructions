...
function paymentHistoryTableHtml(payments = [], currency = 'LKR', invoiceTotal = 0) {
  if (!payments || !payments.length) return '';
  const thStyle = 'border:1px solid #e2e8f0;padding:6px 10px;font-size:8pt;text-transform:uppercase;letter-spacing:0.05em;color:#475569;font-weight:700';
  const tdStyle = 'border:1px solid #e2e8f0;padding:6px 10px;vertical-align:middle;font-size:9pt';
  
  let runningPaid = 0;
  const rows = payments.map((p, i) => {
    runningPaid += Number(p.amount || 0);
    const balAfter = typeof p.remainingBalanceAfter === 'number' ? p.remainingBalanceAfter : Math.max(0, invoiceTotal - runningPaid);
    const dateStr = p.date ? new Date(p.date).toLocaleDateString('en-LK') : '—';
    const typeStr = p.isAdvance ? 'Advance' : 'Installment';
    const methodStr = (p.method || 'cash').replace('_', ' ');
    const refStr = p.reference ? ` (${esc(p.reference)})` : '';
    const recStr = p.receiptNo ? `<span style="font-size:7.5pt;color:#64748b;display:block">${esc(p.receiptNo)}</span>` : '';

    return `
      <tr>
        <td style="${tdStyle};text-align:center">${i + 1}</td>
        <td style="${tdStyle}">${dateStr}${recStr}</td>
        <td style="${tdStyle}"><span style="display:inline-block;padding:2px 6px;border-radius:4px;font-size:7.5pt;font-weight:700;background:${p.isAdvance ? '#f3e8ff;color:#7e22ce' : '#e0f2fe;color:#0369a1'}">${typeStr}</span></td>
        <td style="${tdStyle};text-transform:capitalize">${esc(methodStr)}${refStr}</td>
        <td style="${tdStyle};text-align:right;font-weight:700;color:#15803d">${fmtMoney(p.amount, currency)}</td>
        <td style="${tdStyle};text-align:right;font-weight:600;color:#b91c1c">${fmtMoney(balAfter, currency)}</td>
      </tr>
    `;
  }).join('');
  ...
  return `
    <div style="margin: 20px 0; page-break-inside: avoid;">
      <div style="padding: 6px 12px; background: #0f172a; color: #fff; font-size: 8pt; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; border-radius: 6px 6px 0 0;">
        Payment History & Installment Ledger
      </div>
      <table style="width:100%;border-collapse:collapse;margin:0 0 16px;border:1px solid #e2e8f0;">
        <thead>
          <tr style="background:#f8fafc">
            <th style="${thStyle};text-align:center;width:30px">#</th>
            <th style="${thStyle};text-align:left">Date / Receipt</th>
            <th style="${thStyle};text-align:left">Type</th>
            <th style="${thStyle};text-align:left">Method & Ref</th>
            <th style="${thStyle};text-align:right">Paid Amount</th>
            <th style="${thStyle};text-align:right">Balance Due</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
...
async function getSettings() {
  try {
    ...
    const s = await SiteSetting.findOne().lean().maxTimeMS(2000);
    return s || {};
  } catch {
    return {};
  }
...
async function buildInvoiceDocumentHtml(invoice) {
  ...
  const body = `
    ...
    <div style="font-family:'Segoe UI',system-ui,sans-serif;color:#0f172a;font-size:10.5pt;line-height:1.55;padding:24px 28px;border:1px solid #cbd5e1;border-radius:4px">
      ...
      ${paymentHistoryTableHtml(inv.payments, currency, inv.total)}
...
async function buildPaymentReceiptHtml(invoice, payment) {
  const settings = await getSettings();
  const inv = invoice || {};
  const pay = payment || {};
  const currency = inv.currency || 'LKR';
  ...
  const signaturesHtml = (sigAuth?.data || sigSeal?.data)
    ? `<div style="display:flex;justify-content:flex-end;margin-top:30px;page-break-inside:avoid">
        <div style="width:240px;text-align:center">
          ${sigAuth?.data ? `<img src="${localFileToDataUri(sigAuth.data).replace(/"/g, '')}" style="max-height:60px;margin-bottom:6px;display:block;margin-inline:auto"/>` : '<div style="height:60px"></div>'}
          <div style="border-top:2px solid #0f172a;padding-top:6px;margin-bottom:12px">
            <p style="margin:0;font-weight:700;font-size:9.5pt;text-transform:uppercase">${esc(sigAuth?.name || 'Authorized Officer')}</p>
            ${sigAuth?.title ? `<p style="margin:2px 0 0;font-size:8pt;color:#64748b">${esc(sigAuth.title)}</p>` : ''}
          </div>
          ${sigSeal?.data ? `<img src="${localFileToDataUri(sigSeal.data).replace(/"/g, '')}" style="max-height:90px;display:block;margin-inline:auto"/>` : ''}
        </div>
      </div>`
    : (directorName || sealUrl)
      ? `<div style="margin-top:20px;text-align:right;page-break-inside:avoid">
          ${directorName ? `<p style="font-weight:700;margin:0 0 6px;font-size:10pt">${esc(directorName)}</p>` : ''}
          ${sealUrl ? `<img src="${localFileToDataUri(sealUrl).replace(/"/g, '')}" style="max-height:80px;object-fit:contain"/>` : ''}
          <p style="margin:6px 0 0;font-size:8.5pt;color:#64748b">Authorized Signatory</p>
        </div>`
      : '';

  const body = `
    ${buildLetterhead(settings)}
    <div style="font-family:'Segoe UI',system-ui,sans-serif;color:#0f172a;font-size:10.5pt;line-height:1.55;padding:24px 28px;border:1px solid #cbd5e1;border-radius:6px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;padding-bottom:14px;border-bottom:2px solid #e2e8f0">
        <div>
          <span style="background:#15803d;color:#fff;font-size:8pt;font-weight:800;text-transform:uppercase;padding:4px 8px;border-radius:4px;letter-spacing:0.06em">Official Receipt</span>
          <h2 style="margin:8px 0 2px;font-size:20pt;font-weight:800;letter-spacing:0.04em;color:#0f172a">PAYMENT RECEIPT</h2>
          <p style="margin:0;font-size:10.5pt;font-weight:700;color:#0ea5e9">${esc(receiptNo)}</p>
        </div>
        <div style="text-align:right;font-size:9.5pt;color:#475569">
          <p style="margin:0 0 4px"><span style="color:#94a3b8;font-size:8pt;text-transform:uppercase;font-weight:700;display:block">Receipt Date</span>${esc(payDate)}</p>
          <p style="margin:0"><span style="color:#94a3b8;font-size:8pt;text-transform:uppercase;font-weight:700;display:block">Invoice Ref</span><strong>${esc(inv.invoiceNo || 'N/A')}</strong></p>
        </div>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px">
        <div style="padding:12px 14px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0">
          <p style="margin:0 0 4px;font-size:8pt;font-weight:700;color:#94a3b8;text-transform:uppercase">Received From</p>
          <p style="margin:0;font-weight:700;font-size:10pt">${esc(inv.client?.name || 'Client')}</p>
          ${inv.client?.phone ? `<p style="margin:2px 0 0;font-size:9pt;color:#64748b">Tel: ${esc(inv.client.phone)}</p>` : ''}
          ${inv.client?.email ? `<p style="margin:2px 0 0;font-size:9pt;color:#64748b">${esc(inv.client.email)}</p>` : ''}
        </div>
        <div style="padding:12px 14px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;text-align:right">
          <p style="margin:0 0 4px;font-size:8pt;font-weight:700;color:#94a3b8;text-transform:uppercase">Payment Information</p>
          <p style="margin:0;font-size:9.5pt;color:#475569"><span style="color:#94a3b8">Payment Method:</span> <strong style="text-transform:capitalize">${esc(payMethod)}</strong></p>
          ${pay.reference ? `<p style="margin:3px 0 0;font-size:9pt;color:#64748b"><span style="color:#94a3b8">Reference:</span> ${esc(pay.reference)}</p>` : ''}
          ${inv.project?.title ? `<p style="margin:3px 0 0;font-size:9pt;color:#64748b"><span style="color:#94a3b8">Project:</span> ${esc(inv.project.title)}</p>` : ''}
        </div>
      </div>

      <!-- Amount Highlight Box -->
      <div style="background:#f0fdf4;border:2px solid #86efac;border-radius:8px;padding:16px 20px;margin-bottom:20px;text-align:center">
        <p style="margin:0 0 4px;font-size:8.5pt;font-weight:700;color:#166534;text-transform:uppercase;letter-spacing:0.05em">Amount Received</p>
        <h3 style="margin:0;font-size:24pt;font-weight:900;color:#15803d">${fmtMoney(payAmount, currency)}</h3>
        ${pay.notes ? `<p style="margin:6px 0 0;font-size:9pt;color:#166534;font-style:italic">Note: ${esc(pay.notes)}</p>` : ''}
      </div>

      <!-- Balance Ledger Breakdown -->
      <table style="width:100%;border-collapse:collapse;margin:0 0 20px;font-size:9.5pt;border:1px solid #e2e8f0">
        <thead>
          <tr style="background:#f1f5f9">
            <th style="padding:8px 12px;border:1px solid #e2e8f0;text-align:left;font-size:8pt;text-transform:uppercase;color:#475569">Description</th>
            <th style="padding:8px 12px;border:1px solid #e2e8f0;text-align:right;font-size:8pt;text-transform:uppercase;color:#475569">Amount</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td style="padding:8px 12px;border:1px solid #e2e8f0">Invoice Total (${esc(inv.invoiceNo || 'Invoice')})</td>
            <td style="padding:8px 12px;border:1px solid #e2e8f0;text-align:right;font-weight:600">${fmtMoney(inv.total, currency)}</td>
          </tr>
          <tr>
            <td style="padding:8px 12px;border:1px solid #e2e8f0">Previous Balance Due</td>
            <td style="padding:8px 12px;border:1px solid #e2e8f0;text-align:right;font-weight:600">${fmtMoney(prevBal, currency)}</td>
          </tr>
          <tr style="background:#f0fdf4">
            <td style="padding:8px 12px;border:1px solid #e2e8f0;color:#15803d;font-weight:700">Payment Amount Received (${pay.isAdvance ? 'Advance' : 'Installment'})</td>
            <td style="padding:8px 12px;border:1px solid #e2e8f0;text-align:right;color:#15803d;font-weight:800">- ${fmtMoney(payAmount, currency)}</td>
          </tr>
          <tr style="background:#fff1f2">
            <td style="padding:10px 12px;border:1px solid #e2e8f0;color:#b91c1c;font-weight:800;font-size:10.5pt">Remaining Balance Outstanding</td>
            <td style="padding:10px 12px;border:1px solid #e2e8f0;text-align:right;color:#b91c1c;font-weight:800;font-size:11pt">${fmtMoney(balAfter, currency)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div style="margin-top:20px;border-top:1px solid #e2e8f0;padding-top:10px;text-align:center;font-size:8pt;color:#94a3b8">
        This is an official system generated payment receipt. Thank you for your business!
      </div>
    </div>`;
